...
void insert_into_board()
{
    ...
    {
        vector<string> v_ig_title ;//PRZECHOWUJE DEFINICJE W PIERWSZYM ELEMENCIE I KOLEJNE LITERY ODPOWIEDZI

        v_ig_title.push_back(v_def[i]);//WSADZA DEFINICJE NA PIERWSZE MIEJSCE W VECTORZE

        int v_title_size = v_title[i].size(); // ILOSC LITER W SLOWIE

        string answer = v_title[i];

        for(int z=1; z<v_title_size+1 ;z++)
        {
            v_ig_title.push_back(answer.substr(z-1,1));
        }
        for(int z=0; z<v_title_size+1 ;z++)
        {
            cout<<z<<": ["<<v_ig_title[z]<<"] ";
        }
        cout<<endl;
        ...
        case 1:
            {
                cout<<"x: "<<x_rand<<" y: "<<y_rand<<" pos: horizontal "<<answer<<": "<<v_ig_title.size()<<endl;
                break;
            }
        case 2:
            {
                cout<<"x: "<<x_rand<<" y: "<<y_rand<<" pos: vertical "<<answer<<": "<<v_ig_title.size()<<endl;
                break;
            }
        }

        for(int i=0; i<v_ig_title.size();i++)
        {
            switch(pos)
            {
            case 1: //horizontal [0][y]
                {
                    if(v_ig_title.size()+y_rand>10)
                    {
                        int overflow = v_ig_title.size()+y_rand-10;
                        int sum = v_ig_title.size()+y_rand;
                        y_rand = sum - v_ig_title.size()-overflow;

                        cout<<"HORIZONTAL OVERFLOW"<<endl;

                        v_board[x_rand][i+y_rand] = v_ig_title[i];
                    }
                    else
                    {
                        v_board[x_rand][i+y_rand] = v_ig_title[i];
                ...
                case 2: //vertical [x][0]
                {
                    if(v_ig_title.size()+x_rand>10)
                    {
                        int overflow = v_ig_title.size()+x_rand-10;
                        int sum = v_ig_title.size()+x_rand;
                        x_rand = sum - v_ig_title.size()-overflow;

                        cout<<"VERTICAL OVERFLOW"<<endl;

                        v_board[i+x_rand][y_rand] = v_ig_title[i];
                    }
                    else
                    {
                        v_board[i+x_rand][y_rand] = v_ig_title[i];
